...
def show_cams(img, mask_dic, img_name):
    for name, mask in mask_dic.items():
        show_cam_on_image(img, mask, img_name)
...
def show_cam_on_image(img, mask, name):
    ...
    cam = heatmap 
    cam = cam / np.max(cam)
    cv2.imwrite(out_dir + "{}".format(name), np.uint8(255 * cam))
...
def get_args():
    ...
    parser.add_argument('--image-path', type=str, default='./Dataset/USS_busi',
...
if __name__ == '__main__':
    ...
    image_path = './Dataset/USS_busi_quick/benign/'
    out_dir = './Dataset/CAM_busi/'
    ...
    for obj in os.listdir(image_path):
     ...
     if not(os.path.isfile(out)):
        grad_cam = GradCam(model=model, blob_name = 'layer4', target_layer_names=['2'], use_cuda=False)  # target_layer_names=['35']
        ...
        img_name = obj
        #print(img_name)
        show_cams(img, mask_dic,img_name)
