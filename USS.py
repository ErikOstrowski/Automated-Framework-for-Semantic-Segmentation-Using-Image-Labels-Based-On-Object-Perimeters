...
def run():
    ...
    for obj in os.listdir(pat):
        ...
        if not(os.path.isfile(out)):
            ...
            '''continuity loss'''
            if segment == 'quick':
            	seg_map =  segmentation.quickshift(image, kernel_size=3, max_dist=6, ratio=0.29)
            else:
            	seg_map = segmentation.slic(image,n_segments=1000, compactness=15, sigma=1, start_label=1)
            ...
            for batch_idx in range(args.train_epoch):
                ...
                if un_label.shape[0] < args.max_label_num:  # update show
                    ...
                    # The code below makes the ouput colors similar to the original image, for our use case we don't want that
