...
#######################################################################################################
# Code to Combine results from PerimeterFit with the CAM style of PuzzleCAM
#######################################################################################################
...
if __name__ == '__main__':
    ...
    pred_dir = create_directory(f'./experiments/predictions/Combined_CAM/')
...
# Evaluation
    ...
    with torch.no_grad():
        ...
        for step, (ori_image, image_id, label, gt_mask) in enumerate(dataset):
            ...
            # Conform PerimeterFit.py results with the hr_cams of PuzzleCAM
            ...
            # Combination happens here, we multiply both PerimeterFit.py results with the original result and then add the original result
